...
const buildInitialGeneralValues = (profile: UserProfileResponse | null): GeneralFormValues => ({
    name: profile?.name ?? "",
...
const ProfilePage: React.FC = () => {
    ...
    const setUserProfilePic = profileContext?.setUserProfilePic ?? (() => undefined);

    const [profile, setProfile] = useState<UserProfileResponse | null>(null);
    const [isLoadingProfile, setIsLoadingProfile] = useState<boolean>(true);
    const [generalForm, setGeneralForm] = useState<GeneralFormValues>(() => buildInitialGeneralValues(null));
    const [generalErrors, setGeneralErrors] = useState<Partial<Record<keyof GeneralFormValues, string>>>({});
    ...
    useEffect(() => {
        ...
        const fetchProfile = async () => {
            try {
                ...
                const response = await api.get<UserProfileResponse>(`/user-profile/view/${userId}`, {
                ...
                setProfile(null);
                SEGPrincipalNotificator("Não foi possível carregar seus dados.", "error", "Erro");
            } finally {
                setIsLoadingProfile(false);
            }
        };
    ...
    const handleGeneralChange = (field: keyof GeneralFormValues) => (event: React.ChangeEvent<HTMLInputElement>) => {
        const value = event.target.value;
        setGeneralForm((prev) => ({ ...prev, [field]: value }));
        setGeneralErrors((prev) => ({ ...prev, [field]: undefined }));
    };

    const handlePasswordChange = (field: keyof PasswordFormValues) => (event: React.ChangeEvent<HTMLInputElement>) => {
        const value = event.target.value;
    ...
    const handleSubmitGeneral = async (event: React.FormEvent) => {
        ...
        try {
            ...
            const response = await api.patch<UserProfileResponse>(`/user-profile/update/${userId}`, payload);
            ...
            SEGPrincipalNotificator("Dados atualizados com sucesso!", "success", "Sucesso");
        } catch (error: any) {
            console.error("Erro ao atualizar perfil", error);
            const message = error?.response?.data?.message ?? "Não foi possível atualizar seus dados.";
    ...
    const handleSelectImage = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!file.type.startsWith("image/")) {
            SEGPrincipalNotificator("Selecione um arquivo de imagem válido.", "warning", "Formato inválido");
            return;
        }
    ...
    const handleUploadPicture = async () => {
        ...
        try {
            ...
            const response = await api.post<UserProfileResponse>(`/user-profile/set-profile-picture/${userId}`, formData, {
            ...
            SEGPrincipalNotificator("Foto de perfil atualizada!", "success", "Sucesso");
        } catch (error: any) {
    ...
    const handleSubmitPassword = async (event: React.FormEvent) => {
        ...
        try {
            ...
            SEGPrincipalNotificator("Senha alterada com sucesso!", "success", "Sucesso");
    ...
    const handleDisableAccount = async () => {
        ...
        try {
            ...
            const message = error?.response?.data?.message ?? "Não foi possível desativar a conta.";
            SEGPrincipalNotificator(String(message), "error", "Erro");
        } finally {
    ...
    const isGeneralDirty = useMemo(() => {
        ...
        return (
            ...
            (profile.birth_date ? profile.birth_date.slice(0, 10) : "") !== generalForm.birthDate
        );
    }, [generalForm, profile]);
    ...
    if (!userId) {
        return (
            <Container maxWidth="md">
                <Stack spacing={2} alignItems="center" justifyContent="center" sx={{ py: 12 }}>
                    <Typography variant="h4" fontWeight={700} color={colors.purple} textAlign="center">
                        Faça login para acessar seu perfil
                    </Typography>
                    <Typography variant="body1" color="text.secondary" textAlign="center">
                        Não encontramos informações de usuário ativas. Entre novamente para gerenciar seus dados.
                    </Typography>
                </Stack>
            </Container>
        );
    ...
    return (
        <Container maxWidth="lg" sx={{ py: { xs: 4, md: 6 } }}>
            <Stack spacing={4}>
                <Paper
                    ...
                    sx={{
                        ...
                        border: "1px solid rgba(93,112,246,0.24)",
                    }}
                >
                    <Stack direction={{ xs: "column", md: "row" }} spacing={4} alignItems={{ xs: "center", md: "flex-start" }}>
                        ...
                        <Stack spacing={3} flex={1}>
                            <Box>
                                <Typography variant="overline" color={colors.strongGray} sx={{ letterSpacing: 2 }}>
                                    Olá,
                                </Typography>
                                <Typography variant="h3" fontWeight={800} color={colors.purple} gutterBottom>
                                    {isLoadingProfile ? <Skeleton width={240} /> : `${profile?.name ?? ""} ${profile?.surname ?? ""}`}
                                </Typography>
                                <Typography variant="body1" color="text.secondary" sx={{ maxWidth: 520 }}>
                                    Gerencie suas informações pessoais, proteja sua conta atualizando a senha e personalize a
                                    experiência com uma nova foto de perfil.
                                </Typography>
                            </Box>

                            <Stack direction="row" spacing={1} flexWrap="wrap" alignItems="center">
                                <Chip label={`ID: ${profile?.id_user ?? "--"}`} color="primary" variant="outlined" />
                                <Chip
                                    label={profile?.active === false ? "Conta desativada" : "Conta ativa"}
                            ...
                            {isUploadingPicture && <LinearProgress sx={{ borderRadius: 999 }} />}
                        </Stack>
                    </Stack>
                </Paper>
                ...
                <Grid container spacing={4} alignItems="stretch">
                    <Grid item xs={12} lg={7}>
                        <Paper elevation={0} sx={{ borderRadius: 4, p: { xs: 3, md: 4 }, height: "100%" }}>
                            <Stack component="form" spacing={3} onSubmit={handleSubmitGeneral} height="100%">
                                <Box>
                                    <Typography variant="h5" fontWeight={700} color={colors.purple} gutterBottom>
                                        Informações pessoais
                                    </Typography>
                                    <Typography variant="body2" color="text.secondary">
                                        Atualize seus dados básicos. Essas informações são usadas para personalizar sua experiência
                                        no sistema.
                                    </Typography>
                                </Box>
                    ...
                    <Grid item xs={12} lg={5}>
                        <Paper elevation={0} sx={{ borderRadius: 4, p: { xs: 3, md: 4 }, height: "100%" }}>
                            <Stack component="form" spacing={3} onSubmit={handleSubmitPassword} height="100%">
                                <Box>
                                    <Typography variant="h5" fontWeight={700} color={colors.purple} gutterBottom>
                                        Segurança e acesso
                                    </Typography>
                                    <Typography variant="body2" color="text.secondary">
                                        Altere sua senha periodicamente para manter sua conta protegida. Confirme a nova senha antes
                                        de salvar.
                                    </Typography>
                                </Box>
                ...
                >
                    <Stack spacing={3}>
                        <Stack direction="row" spacing={2} alignItems="center">
                            <WarningAmberRoundedIcon sx={{ color: "#F26D6D", fontSize: 36 }} />
                            <Box>
                                <Typography variant="h6" fontWeight={700} color="#C0392B">
                                    Zona de risco
                                </Typography>
                                <Typography variant="body2" color="text.secondary">
                                    Desativar sua conta remove seu acesso imediato ao sistema. Você poderá solicitar a reativação
                                    posteriormente junto à instituição.
                                </Typography>
                            </Box>
                        </Stack>
            ...
            <Dialog open={disableDialogOpen} onClose={() => (!isDisablingAccount ? setDisableDialogOpen(false) : undefined)}>
                <DialogTitle>Deseja realmente desativar sua conta?</DialogTitle>
                <DialogContent>
                    <Typography variant="body2" color="text.secondary">
                        Essa ação pode ser revertida somente pelo time administrativo. Confirme para prosseguir com a
                        desativação.
                    </Typography>
                </DialogContent>
                <DialogActions sx={{ px: 3, pb: 3 }}>
                    <Button onClick={() => setDisableDialogOpen(false)} disabled={isDisablingAccount}>
                        Cancelar
                    </Button>
                    <SEGButton
                        colorTheme="purple"
